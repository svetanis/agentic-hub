...
#### 1. **MultiAgentsSystem** - Entry Point
  - Main application class that initializes and starts the unified system
  - Starts a web server hosting all root agents
  - **Command to run:** `mvn compile exec:java -Dexec.mainClass="com.svetanis.agents.MultiAgentsSystem"`
...
All agent configurations are stored in YAML format in `src/main/resources/agent-configs/blogger/`. Each agent uses Groq's Llama 3.3 70B Versatile model.
...
```
src/main/java/com/svetanis/agents/blogger/
